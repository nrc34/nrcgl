...
namespace nrcgl
{
	public class GLView : AndroidGameView
	{
		...
		protected override void OnLoad (EventArgs e)
		{
			...
			Texture.LoadTexture (Context, Resource.Drawable.space256x256, texturePanelId);
			...
			Panel2.Rotate (Vector3.UnitY, MathHelper.Pi);
			Panel2.Scale = new Vector3 (4);
			Panel2.Position = 
				new Vector3 (Panel2.Position.X,
					Panel2.Position.Y - 3,
					Panel2.Position.Z + 8f);
...
//			CurrShape.Scale = new Vector3 (scale);
			...
			Shapes3D ["panel1"].Position = 
				...
				Shapes3D ["panel1"].Position.Z - 0.001f);

			Shapes3D ["panel2"].Position = 
				new Vector3 (
					Shapes3D ["panel2"].Position.X,
					Shapes3D ["panel2"].Position.Y,
					Shapes3D ["panel2"].Position.Z - 0.001f);

			if (Shapes3D ["panel2"].Position.Z < -8f)
				Shapes3D ["panel2"].Position = new Vector3 (
					Shapes3D ["panel2"].Position.X,
					Shapes3D ["panel2"].Position.Y,
					Shapes3D ["panel1"].Position.Z + 8f); 

			if (Shapes3D ["panel1"].Position.Z < -8f)
				Shapes3D ["panel1"].Position = new Vector3 (
					Shapes3D ["panel1"].Position.X,
					Shapes3D ["panel1"].Position.Y,
					Shapes3D ["panel2"].Position.Z + 8f); 
